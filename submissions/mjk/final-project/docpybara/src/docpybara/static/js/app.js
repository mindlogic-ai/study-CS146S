/**
 * Main application logic for docpybara.
 */
...
async function init() {
    ...
    loadCodeMirror()
        .then(() => {
            setupAI();
            console.log('docpybara: CodeMirror loaded');
        })
        .catch(err => console.error('docpybara: CodeMirror load failed:', err));
}

init().catch(err => console.error('docpybara init failed:', err));
