...
async function coinGeckoRequest<T>(path: string, params: Record<string, string> = {}): Promise<T> {
    ...
    try {
      ...
      if (response.status === 429) {
        if (attempt < MAX_RETRIES) {
          ...
          console.error(`Rate limited (429). Retrying in ${backoffMs}ms (attempt ${attempt}/${MAX_RETRIES})...`);
          await sleep(backoffMs);
          continue;
        }
        throw new Error("Rate limit exceeded after retries. Please wait before making another request.");
      }
      ...
      if ((err as Error).name === "AbortError") {
        if (attempt < MAX_RETRIES) {
          console.error(`Request timed out. Retrying (attempt ${attempt}/${MAX_RETRIES})...`);
          continue;
        }
        throw new Error("Request timed out after retries. The CoinGecko API may be slow or unreachable.");
      }
      throw err;
    } finally {
...
// Tool 1: Get market prices for top coins or specific coins
server.registerTool(
  "get_coin_markets",
  {
    title: "Get Coin Markets",
    description:
      "Get current market data (price, market cap, volume, 24h change) for cryptocurrencies. " +
      "Can filter by specific coin IDs or get top coins by market cap.",
    inputSchema: {
      vs_currency: z
        .string()
        .default("usd")
      ...
      ids: z
        .string()
        .optional()
        .describe("Comma-separated coin IDs (e.g. bitcoin,ethereum). Leave empty for top coins."),
      per_page: z
        .number()
        .min(1)
        .max(50)
  ...
  async ({ vs_currency, ids, per_page }) => {
    try {
      ...
      const data = await coinGeckoRequest<CoinMarket[]>("/coins/markets", params);
...
// Tool 2: Get detailed info for a specific coin
server.registerTool(
  "get_coin_detail",
  {
    title: "Get Coin Detail",
    description:
      "Get detailed information about a specific cryptocurrency including description, " +
      "market data, price changes, and supply info.",
    inputSchema: {
      coin_id: z
        .string()
  ...
  async ({ coin_id, vs_currency }) => {
    try {
      const data = await coinGeckoRequest<CoinDetail>(`/coins/${encodeURIComponent(coin_id)}`, {
        localization: "false",
        tickers: "false",
...
server.registerTool(
  "get_trending",
  {
    ...
    description: "Get the top trending cryptocurrencies on CoinGecko based on search activity.",
    inputSchema: {},
  },
  async () => {
    try {
      const data = await coinGeckoRequest<TrendingResponse>("/search/trending");

      if (data.coins.length === 0) {
        return { content: [{ type: "text", text: "No trending coins found." }] };
      }
      ...
      return {
        content: [{ type: "text", text: "# Trending Coins\n\n" + lines.join("\n") }],
      };
    } catch (err) {
      return {
        content: [{ type: "text", text: `Error: ${(err as Error).message}` }],
